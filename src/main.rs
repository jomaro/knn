...
fn read_files(a : &str, b : &str) -> Data {
	...
	Data {d: d, c: c, data: data, 
			rotulos: rotulos, test: test, rotulos_test: rotulos_test}
}


fn predict(x : &[f64], k : usize, data_ : &Data) -> usize {
	
	let d = data_.d as usize;
	let c = data_.c as usize;
	
	let data = &data_.data;
	let rotulos = &data_.rotulos;
	
	
	let mut viz = Vec::<(f64, usize)>::with_capacity(k);
...
fn main(){
	...
	let rotulos_tests : Vec<usize> = data.test
		.chunks(data.d as usize)
...
/*	
	...
	let rotulos_testp : Vec<usize> = data.test
		.chunks(data.D as usize).par_iter()
//		.inspect(|x| print!("{:?}", x))
//		.flatten().map(|x| *x as usize).collect();
		.map(|x| predict(&x, k, &data))
		.collect();
		
	println!("Paralelo: {:?}", Instant::now().duration_since(now));
*/	
	println!("{} missmatches from {}", data.rotulos_test.iter()
		.zip(rotulos_tests)
//		.zip(rotulos_testp)
		.filter(|&(a, b)| *a as usize != b)
		.inspect(|&(a, b)| println!("{} {}", a, b))
		.count(), data.rotulos_test.len());
